...
if isequal(file,0)
   ...
   if length(nifti_array) == 4
       ...
       % Iterate Through Vol
       parfor current_volume =1: total_volumes
           slice_counter = 0;
   ...
   elseif length(nifti_array) == 3
       ...
       slice_counter = 0;
        % Iterate Through Slices
        %current_slice = 1;
        parfor current_slice = 1:total_slices
            % Alternate Slices
            %if mod(slice_counter, 1) == 0     
                ...
                percentage = strcat('Please wait. Converting...', ' ', num2str(100*current_slice/total_slices), '% Complete');
